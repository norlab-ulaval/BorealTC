...
from utils import models, preprocessing, transforms
...
RANDOM_STATE = 21
rho = os.environ.get("RHO", False)
rho = True
ablate = False
desc = f"{'rho' if rho else 'nhr'}{'abl' if ablate else 'nab'}"
...
if rho:
    pro_df = terr_dfs["pro"]
    pro_df["vx"], pro_df["wz"] = transforms.unicycle_model(pro_df)
    pro_df["rho"] = transforms.rotationnality(pro_df)
    ign_cols = ["terrain", "run_idx"]
    pro_cols = [c for c in pro_df.columns if c not in ign_cols] + ign_cols
    terr_dfs["pro"] = pro_df[pro_cols].copy()

if ablate:
    pro_df = terr_dfs["pro"]
    pro_cols = [c for c in pro_df.columns if "vel" not in c]
    terr_dfs["pro"] = pro_df[pro_cols].copy()
...
for mw in MOVING_WINDOWS:
    ...
    for model in BASE_MODELS:
        ...
        print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        result_path = results_dir / f"results_{model}_{desc}_mw_{mw}.npy"
