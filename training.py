...
if __name__ == '__main__':
    ...
    datamodule = VulpiDataModule(root_dir=Path('datasets'), batch_size=batch_size, transform=to_spectrogram, num_workers=num_workers)
    ...
    trainer = L.Trainer(accelerator='gpu', precision=32, logger=logger, log_every_n_steps=1,
                        ...
                        callbacks=[
                            # EarlyStopping('val_loss', patience=15, verbose=True),
                            DeviceStatsMonitor(),
                            LearningRateMonitor(),
                            ModelCheckpoint(
                                monitor='val_loss',
