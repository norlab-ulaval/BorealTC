...
cc = confusionchart(RES.CNN.SampWindow_1500ms.ConfusionMat);
cc.RowSummary = 'row-normalized';
cc.ColumnSummary = 'column-normalized';

acc = 0;
for i = 1:size(RES.CNN.SampWindow_1500ms.ConfusionMat,1)
    acc = acc + RES.CNN.SampWindow_1500ms.ConfusionMat(i,i);
end

acc = 100*(acc/sum(sum(RES.CNN.SampWindow_1500ms.ConfusionMat)));
